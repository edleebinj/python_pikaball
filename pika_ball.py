    ...
    def smash_from_left(self,ballbody):
        if pygame.key.get_pressed()[K_z]:
            if pymunk.Vec2d.get_distance(ballbody.position,pymunk.Vec2d(
                    self.rect.x + PIKAWIDTH / 2 ,self.rect.y +PIKAHEIGHT / 2)) < BALLRADIUS+30:
                shortsmash = True
                angle = None
                ...
                if shortsmash == False:
                    ballbody.velocity = pymunk.Vec2d(200+1.03 * ballbody.velocity.__abs__() * math.cos(degree_angle),
                                                     -1 * ballbody.velocity.__abs__() * math.sin(degree_angle))
